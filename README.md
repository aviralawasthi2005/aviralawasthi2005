

<div align="center">

# Hi, I'm Aviral Awasthi 👋

### Software Engineer · Full-Stack Developer · Backend Enthusiast

I build scalable web applications, backend systems, and real-time experiences using modern JavaScript technologies. I care about clean architecture, performance, and user experience.

<br/>


[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](#)
[![Twitter](https://img.shields.io/badge/Twitter-000000?style=for-the-badge&logo=x&logoColor=white)](#)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](#)

</div>

<br/>

## ⚡ Tech Stack

<div align="center">

**Languages**

![JavaScript](https://img.shields.io/badge/JavaScript-18181B?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![TypeScript](https://img.shields.io/badge/TypeScript-18181B?style=for-the-badge&logo=typescript&logoColor=3178C6)
![Python](https://img.shields.io/badge/Python-18181B?style=for-the-badge&logo=python&logoColor=3776AB)
![Java](https://img.shields.io/badge/Java-18181B?style=for-the-badge&logo=openjdk&logoColor=ED8B00)

**Frontend**

![React](https://img.shields.io/badge/React-18181B?style=for-the-badge&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-18181B?style=for-the-badge&logo=nextdotjs&logoColor=FFFFFF)
![Tailwind](https://img.shields.io/badge/Tailwind-18181B?style=for-the-badge&logo=tailwindcss&logoColor=06B6D4)

**Backend**

![Node.js](https://img.shields.io/badge/Node.js-18181B?style=for-the-badge&logo=nodedotjs&logoColor=5FA04E)
![Express](https://img.shields.io/badge/Express-18181B?style=for-the-badge&logo=express&logoColor=FFFFFF)
![Socket.io](https://img.shields.io/badge/Socket.io-18181B?style=for-the-badge&logo=socketdotio&logoColor=FFFFFF)

**Database & Infrastructure**

![MongoDB](https://img.shields.io/badge/MongoDB-18181B?style=for-the-badge&logo=mongodb&logoColor=47A248)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-18181B?style=for-the-badge&logo=postgresql&logoColor=4169E1)
![Redis](https://img.shields.io/badge/Redis-18181B?style=for-the-badge&logo=redis&logoColor=DC382D)
![Docker](https://img.shields.io/badge/Docker-18181B?style=for-the-badge&logo=docker&logoColor=2496ED)
![AWS](https://img.shields.io/badge/AWS-18181B?style=for-the-badge&logo=amazonwebservices&logoColor=FF9900)

</div>

<br/>

## 🚀 Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### ◉ Voxa — Real-Time Messaging

A full-stack real-time messaging platform with one-to-one and group conversations.

**`MERN` `Socket.io` `JWT` `MongoDB` `Tailwind CSS`**

- 💬 Real-time messaging
- 👥 Group conversations
- 🟢 Online presence
- ⌨️ Typing indicators
- ✅ Read receipts
- 🔔 Real-time notifications

</td>
<td width="50%" valign="top">

### ◉ Real Estate Property Listing

A full-stack real estate platform for discovering, listing, and managing properties with a responsive interface.

**`React` `Node.js` `Express` `MongoDB`**

- 🏠 Property listings
- 🔍 Search & filtering
- 📋 Property details
- 🔐 User authentication
- 🛠️ Property management
- 📱 Responsive design

</td>
</tr>
</table>

<br/>

## 🧩 What I Like Building

<div align="center">

```
Real-time applications
        ↓
    Backend APIs
        ↓
Scalable web applications
        ↓
   Cloud & DevOps
        ↓
  AI-powered products
```

</div>

<br/>

## 📊 GitHub Stats

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117" />

<img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_USERNAME&theme=tokyonight&hide_border=true&background=0D1117" />

</div>

<br/>

<div align="center">

### 💭 "Build things that scale, and code that's easy to read tomorrow."

</div>


